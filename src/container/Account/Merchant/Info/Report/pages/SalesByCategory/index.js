...
const Index = () => {
  ...
  const getData = (quickFilter = "", start = "", end = "") => {
    let url = `product/report/saleByCategory?quickFilter=${quickFilter}&timeStart=${start}&timeEnd=${end}&merchantId=${merchantId}`;
    url = encodeURI(url);
    dispatch(getSalesByCategory(url, token));
  };

  const exportData = (quickFilter = "", start = "", end = "", type = "") => {
    let url = `product/report/saleByCategory/export?quickFilter=${quickFilter}&timeStart=${start}&timeEnd=${end}&merchantId=${merchantId}&type=${type}`;
    url = encodeURI(url);
  ...
  const onClickSort = (direction, type) => {
    dispatch(sort_sales_by_category({ type }));
  };

  return (
    <>
      <div className="info_merchant_title">Sales by Category</div>
      <SelectDate
        value={valueDate}
      ...
      <div className="table-container">
        <ReactTable
          ...
          loading={loading}
          columns={columns(directionSort_sales_by_category, onClickSort)}
